...
- **Cast for conflict — a clear trio.** Most landing scenes run on three roles in tension: the **everyman** (whose stake we feel), the **foil** (confidently wrong — the comedy engine), and the **hero** (the brand's POV, who resolves it). Three roles in conflict carry a tight 15–20s scene with zero narration.
...
- **FLOW** — the beat-by-beat shot flow as an EXPLICIT 6-COLUMN TABLE, one row per cut. REQUIRED handoff, never optional — your written dialogue + direction drive the acting far better than lines inferred downstream, and the SHOT SPEC column feeds both the board's per-frame annotation and `video-prompt`. Columns:
  ...
  - **Dialogue** — the actual spoken line, **prefixed with the speaker's name** (`Maya: "Not if you learn to use AI like me."`). Always label WHO speaks — every line, even in a one-character video — so `video-prompt` knows whose mouth moves. Use "—" ONLY for true silent beats; do NOT leave blank to fill later. These exact lines get spoken (native audio nails scripted lines), so write them tight, natural, and in-character (see DIALOGUE VOICE below).
  ...
  **DIALOGUE VOICE — natural + standup-funny (write it the way people actually talk). MANDATORY GATE — no script ships until every line passes.** Coherent isn't enough; native audio bakes these EXACT words and delivery into the clip, and you cannot fix a stiff, written, robotic, or AI-sounding line after generation. So this is a hard gate, not a polish: read every line aloud, and if a single line sounds like writing instead of speech, REWRITE it before handoff. The lines must sound like real humans talking and land like a tight comedy bit. Run this voice pass on the Dialogue column:
  ...
  - **Standup cadence — setup → punchline.** Build like a comedy bit: a straight setup line, then a turn, then the **punch lands on the LAST few words** (never bury the funny word mid-sentence). Use the foil's confident-wrong lines as the joke engine, and **end the scene on the strongest button** (the callback from Phase 4B). Comedy is timing: keep setups lean so the punch hits fast, and give the punchline its own beat/cut.
...
… (one line per cut; name the speaker every line; "—" for silent beats)
...
3. Hook lab
4. Improved concept (incl. Phase 4B script devices — close the loop, callback button, confident-wrong foil, personify the concept as a named character, cast a conflict trio; and the ANTI-CORNY CONCEPT GATE — extract the reference's mechanic not its nouns, cold-open quarantine for 8–15s, brand reveal as the format's next natural line, no corporate-metaphor hook, absurdity-with-rules, jargon gate). If a paid Meta ad: apply the META BUSINESS ADS layer (one generated ad by default; 3 text alternate hooks to test later, proof beat, explicit button-matched CTA, burned-in captions, placement safe zones, ad-policy-safe lines).
5. 9-beat shot structure
6. Quality gate summary (type-appropriate)
7. Clarification checkpoint — confirm DIRECTION and ask any open questions (skip with one line if already clear and approved). Honor CONCEPT-FIRST MODE (no images/board/handoff until the user asks to build); if the user rejects a concept, diagnose the mechanic mismatch and rebuild — don't defend the failed idea.
8. Board prompt + text breakdown (Step 8-i) — show the finalized image-gen prompt for the landscape board (shared choices, character reference, environment + floor plan, storyboard strip, lighting/mood) AND the text breakdown (video concept, shared choices, characters, environment, 6-column flow table, editor notes, style notes, brand/logo notes); state assumptions and refine with the user. No image yet. Before moving to generate, run the **dialogue-only coherence pass** AND the **dialogue-voice pass** on the FLOW table (Phase 8B) — read just the Dialogue column top-to-bottom: one through-line, every line answers/escalates the last, no contradictions; then read it aloud so every line sounds like real natural speech (contractions, one consistent voice per labeled speaker, correct idiom) and lands with standup setup→punchline timing (punch on the last words, scene ends on the strongest button). Rewrite the lines (never just reorder clips) if either fails.
